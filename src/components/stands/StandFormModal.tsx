...
const StandFormSchema = z.object({
  ...
  time_of_day: z.union([z.enum(['AM', 'PM', 'ALL']), z.literal(''), z.null()]).optional(),
  view_distance_yards: z.number().min(0).max(1000).nullable(),
  nearby_water_source: z.boolean().nullable(),
  food_source: z.union([z.enum(['field', 'feeder']), z.literal(''), z.null()]).optional(),
  ...
  const onFormSubmit = async (data: StandFormData) => {
    try {
      ...
      const dbData = {
        ...
        time_of_day: (data.time_of_day === '' ? null : (data.time_of_day ?? null)) as 'AM' | 'PM' | 'ALL' | null,
        view_distance_yards: data.view_distance_yards,
        nearby_water_source: data.nearby_water_source,
        food_source: (data.food_source === '' ? null : (data.food_source ?? null)) as 'field' | 'feeder' | null,
